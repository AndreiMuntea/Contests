...
public:
    ...
    void Update(size_t Low, size_t High, int Value)
    {
        Update(1, 0, Array.size() - 1, Low, High, Value);
    }
...
private:
    ...
    void Update(size_t Node, size_t Start, size_t End, size_t Low, size_t High, int Value)
    {
        ...
        Update(Node * 2, Start, mid, Low, High, Value);
        Update(Node * 2 + 1, mid + 1, End, Low, High, Value);
...
int main()
{
    ...
    {
        cin >> t >> r >> l;
        if (t == 0) tree.Update(r - 1, r - 1, l);
        else cout << tree.Query(r - 1, l - 1) << '\n';
    }
